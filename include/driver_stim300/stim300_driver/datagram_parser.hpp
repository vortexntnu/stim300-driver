...
struct SensorConfig {
  ...
  bool operator!=(const SensorConfig& rhs) const {
    return sample_freq != rhs.sample_freq ||
           datagram_id != rhs.datagram_id ||
...
public:
  DatagramParser(DatagramIdentifier dg_id,
                 GyroOutputUnit gyro_o,
                 AccOutputUnit acc_o,
...
private:
  void set_data_scales(GyroOutputUnit gyro_o,
                       AccOutputUnit acc_o,
                       InclOutputUnit incl_o,
                       AccRange acc_range);
